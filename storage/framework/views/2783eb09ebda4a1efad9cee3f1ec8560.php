...
<div class="container py-4 px-3 px-md-4">
    ...
    <?php
        $hasCancelButton = in_array($appointment->status, ['pending','accepted','waiting_examination']);
        $hasPaymentButton = $appointment->payment_status !== \App\Models\Appointment::PAYMENT_STATUS_PAID && in_array($appointment->status, ['pending','accepted']);
        $hasPaymentAlert = $appointment->payment_status !== \App\Models\Appointment::PAYMENT_STATUS_PAID && in_array($appointment->status, ['pending','accepted']);
        $hasRefundAlert = in_array($appointment->status, ['pending','accepted','waiting_examination']) && $appointment->payment_status === 'paid';
        $hasContent = $hasCancelButton || $hasPaymentButton || $hasPaymentAlert || $hasRefundAlert;
    ?>
